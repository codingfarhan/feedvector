import { ClientInformation, PostDetails, PostResponse } from "@gitroom/nestjs-libraries/integrations/social/social.integrations.interface"
...
export class MastodonCustomProvider extends MastodonProvider {
  ...
  async externalUrl(url: string) {
    ...
    form.append("client_name", "FeedVector")
  ...
  override async generateAuthUrl(refresh?: string, external?: ClientInformation) {
    const state = makeId(6)
    const url = this.generateUrlDynamic(external?.instanceUrl!, state, external?.client_id!, process.env.FRONTEND_URL!, refresh)
  ...
  override async authenticate(
    ...
    return this.dynamicAuthenticate(clientInformation?.client_id!, clientInformation?.client_secret!, clientInformation?.instanceUrl!, params.code)
  }

  override async post(id: string, accessToken: string, postDetails: PostDetails[]): Promise<PostResponse[]> {
