...
export class TelegramProvider extends SocialAbstract implements SocialProvider {
  ...
  async authenticate(params: { code: string; codeVerifier: string; refresh?: string }) {
    ...
    const photo = !chat?.photo?.big_file_id ? "" : await telegramBot.getFileLink(chat.photo.big_file_id)
  ...
  async getBotId(query: { id?: number; word: string }) {
    ...
    const match = res.find(
      (p) =>
        (p?.message?.text === `/connect ${query.word}` && p?.message?.chat?.id) ||
        (p?.channel_post?.text === `/connect ${query.word}` && p?.channel_post?.chat?.id),
    )
    ...
    return chatId
      ...
      : res.length > 0
      ? {
          lastChatId: res?.[res.length - 1]?.message?.chat?.id || res?.[res.length - 1]?.channel_post?.chat?.id,
        }
      : {}
  }
  ...
  private async sendMessage(accessToken: string, message: PostDetails, replyToMessageId?: number): Promise<number | null> {
    ...
    // if there are multiple media, bot sends them as a media group - max 10 media per group - with the text as a caption (if there are more than 1 group, the caption will only be sent with the first group)
      ...
      for (let i = 0; i < mediaGroups.length; i++) {
        ...
        const response = await telegramBot.sendMediaGroup(accessToken, mediaGroup as any[], {
  ...
  async post(id: string, accessToken: string, postDetails: PostDetails[]): Promise<PostResponse[]> {
    ...
    // for private groups/channels message.id is undefined so the link generated by FeedVector will be unusable "https://t.me/c/undefined/16"
