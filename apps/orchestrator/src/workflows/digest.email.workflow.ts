...
export async function digestEmailWorkflow({ organizationId, queue = [] }: { organizationId: string; queue?: Email[] }) {
  ...
  while (true) {
    ...
    for (const user of org.users) {
      ...
      const toSend = batch.filter((email) => email.type === allowFailure || email.type === allowSuccess || email.type === "info")

      if (toSend.length === 0) continue

      await sendEmailAsync(
        user.user.email,
        toSend.length === 1 ? toSend[0].title : `[FeedVector] Your latest notifications`,
