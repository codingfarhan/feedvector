...
const WalletProvider = dynamic(() => import("@gitroom/frontend/components/auth/providers/wallet.provider"), {
  ssr: false,
...
export function RegisterAfter({ token, provider }: { token: string; provider: string }) {
  const t = useT()
  const { isGeneral, genericOauth, neynarClientId, billingEnabled } = useVariables()
