    ...
    <FormProvider {...form}>
      <form onSubmit={form.handleSubmit(submit)}>
        <div className="relative flex gap-[10px] flex-col flex-1 p-[16px] pt-0">
          {sendEmail && <Input label="Email" placeholder={t("enter_email", "Enter email")} name="email" />}
          ...
          <div className="flex gap-[5px]">
            ...
            <div>{t("send_invitation_via_email", "Send invitation via email?")}</div>
          </div>
...
export const TeamsComponent = () => {
  ...
  const getLevel = useCallback((role: "USER" | "ADMIN" | "SUPERADMIN") => (role === "USER" ? 0 : role === "ADMIN" ? 1 : 2), [])
  ...
  const remove = useCallback(
    (toRemove: {
      ...
      async () => {
        if (!(await deleteDialog(t("are_you_sure_remove_team_member", "Are you sure you want to remove this team member?")))) {
          return
        }
        ...
        <div className="flex flex-col gap-[16px]">
          {(data || []).map((p) => (
            <div key={p.user.id} className="flex items-center">
              <div className="flex-1">{capitalize(p.user.email.split("@")[0]).split(".")[0]}</div>
              <div className="flex-1">
                {p.role === "USER" ? t("user", "User") : p.role === "ADMIN" ? t("admin", "Admin") : t("super_admin", "Super Admin")}
              </div>
