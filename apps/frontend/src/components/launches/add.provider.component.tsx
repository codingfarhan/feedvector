...
export const CustomVariables: FC<{
  ...
  const schema = useMemo(() => {
    return object({
      ...variables.reduce((aIcc, item) => {
        ...
        return {
          ...aIcc,
          [item.key]: string().matches(regex, `${item.label} is invalid`).required(),
        }
      }, {}),
    })
  }, [variables])
  ...
  return (
    <div className="rounded-[4px] relative">
      <FormProvider {...methods}>
        <form className="gap-[8px] flex flex-col pt-[10px]" onSubmit={methods.handleSubmit(submit)}>
          {variables.map((variable) => (
            <div key={variable.key}>
              <Input label={variable.label} name={variable.key} type={variable.type == "text" ? "text" : "password"} />
            </div>
          ))}
          <div>
            <Button type="submit">{t("connect", "Connect")}</Button>
  ...
  return (
    <div className="flex flex-col gap-[16px] pt-[8px]">
      <p className="text-[14px] text-textColor/80">
        {t("extension_not_available", "The FeedVector browser extension is not installed. You need to install it before connecting this channel.")}
      </p>
      <div className="flex gap-[10px]">
        <Button
          ...
          className="flex-1"
          onClick={() => {
            window.open("https://chromewebstore.google.com/detail/postiz/cidhffagahknaeodkplfbcpfeielnkjl?hl=en", "_blank")
            modals.closeCurrent()
          }}
        >
          {t("install_extension", "Install Extension")}
        </Button>
        <Button type="button" className="flex-1 !bg-transparent border border-tableBorder text-textColor" onClick={() => modals.closeCurrent()}>
          {t("cancel", "Cancel")}
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col gap-[16px] pt-[8px]">
      ...
      <ul className="flex flex-col gap-[8px] list-disc ps-[20px] text-[14px] text-textColor/80">
        ...
        <li>{t("chrome_extension_warning_reconnect", "You may need to reconnect periodically if the session expires.")}</li>
        <li>We will store your cookies securely to facilitate the connection.</li>
        <li>FeedVector does not take responsibility for any issues arising or account termination due to the use of this method.</li>
      </ul>
    ...
    (
      ...
      async () => {
        ...
        const gotoIntegration = async (externalUrl?: string) => {
          ...
          if (invite) {
            toaster.show("Invite link copied to clipboard, link will be available for 1 hour", "success")
            modal.closeAll()
            copy(url)
            return
          }
        ...
        if (isChromeExtension) {
          ...
          try {
            ...
            toaster.show(t("extension_not_installed", "FeedVector browser extension is not installed or not reachable."), "warning")
            return
          }
          try {
            const cookieResponse = await new Promise<any>((resolve, reject) => {
              chrome.runtime.sendMessage(extensionId, { type: "GET_COOKIES", provider: identifier }, (response: any) => {
            ...
            if (!cookieResponse.success) {
              toaster.show(
                cookieResponse.error || t("extension_cookies_missing", "Could not get cookies. Please log in to the platform first."),
                "warning",
              )
              return
            }
        ...
        if (customFields) {
          modal.openModal({
            ...
            children: (
              <CustomVariables identifier={identifier} gotoUrl={(url: string) => router.push(url)} variables={customFields} onboarding={onboarding} />
            ),
          })
          return
        }
        ...
        >
          {social
            ...
            .filter((item) => {
              ...
              return !item.isExternal && !item.isWeb3 && !item.isChromeExtension && !item.customFields
            })
            .map((item) => (
              <div
                key={item.identifier}
