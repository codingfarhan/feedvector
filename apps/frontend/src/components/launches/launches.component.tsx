...
> = (props) => {
  const { group, mutate, update, continueIntegration, totalNonDisabledChannels, refreshChannel, changeItemGroup, collapsed } = props
  const [isOpen, setIsOpen] = useState(!!+(localStorage.getItem(group.name + "_isOpen") || "1"))
...
> = (props) => {
  const { totalNonDisabledChannels, continueIntegration, refreshChannel, mutate, update, integration, collapsed } = props
  const user = useUser()
  const t = useT()
    ...
    >
      <div className={clsx("relative gap-[6px] flex justify-center items-center", integration.disabled && "opacity-50")}>
        ...
        {(integration.inBetweenSteps || integration.refreshNeeded) && (
          <div
            ...
            onClick={integration.refreshNeeded ? refreshChannel(integration) : continueIntegration(integration)}
          >
        ...
        {integration.identifier === "youtube" ? (
          <img src="/icons/platforms/youtube.svg" className="absolute z-10 bottom-[5px] -end-[5px]" width={20} />
        ) : (
          <Image
      ...
      <div
        ...
        {...(integration.disabled && totalNonDisabledChannels === user?.totalChannels
          ? {
              "data-tooltip-id": "tooltip",
              "data-tooltip-content": t("channel_disabled_upgrade_plan", "This channel is disabled, please upgrade your plan to enable it."),
            }
          : {})}
        role="Handle"
        className={clsx(
      ...
      <Menu
        ...
        onChange={update}
        id={integration.id}
        canEnable={user?.totalChannels! > totalNonDisabledChannels && integration.disabled}
...
export const LaunchesComponent = () => {
  ...
  const menuIntegrations = useMemo(() => {
    return orderBy(
      Object.values(groupBy(sortedIntegrations, (o) => o?.customer?.id || "")).map((p) => ({
        name: (p[0].customer?.name || "") as string,
        id: (p[0].customer?.id || "") as string,
        isEmpty: p.length === 0,
        values: orderBy(p, ["type", "disabled", "identifier"], ["desc", "asc", "asc"]),
      })),
      ["isEmpty", "name"],
      ["desc", "asc"],
    )
  ...
  const refreshChannel = useCallback(
    (
      ...
      async () => {
        const { url } = await (
          await fetch(`/integrations/social/${integration.identifier}?refresh=${integration.internalId}`, {
            method: "GET",
          })
        ).json()
