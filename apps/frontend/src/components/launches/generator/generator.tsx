...
const FirstStep: FC = (props) => {
  ...
  const generateStep = useCallback(
    async (reader: ReadableStreamDefaultReader) => {
      ...
      while (true) {
        ...
        for (const chunk of chunkStr.split("\n").filter((f) => f && f.indexOf("{") > -1)) {
          try {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className={loading ? "pointer-events-none select-none opacity-75" : ""}>
      <FormProvider {...form}>
          ...
          <div className="pb-[10px] rounded-[4px]">
            <div className="flex">
              <div className="flex-1">
                ...
                />
                <Select label={t("output_format", "Output format")} {...form.register("format")}>
                ...
                <Select label={t("output_format", "Output format")} {...form.register("tone")}>
                  <option value="personal">{t("personal_voice_i_am_happy_to_announce", 'Personal voice ("I am happy to announce")')}</option>
                  <option value="company">{t("company_voice_we_are_happy_to_announce", 'Company voice ("We are happy to announce")')}</option>
                </Select>
                <div className={clsx("flex items-center", loading && "opacity-50")}>
                  <Checkbox disabled={loading} {...form.register("isPicture")} label={t("add_pictures", "Add pictures?")} />
                </div>
              </div>
            </div>
          </div>
