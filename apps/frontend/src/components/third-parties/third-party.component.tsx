...
export const ThirdPartyMenuComponent: FC<{
  ...
  const deleteChannel = (id: string) => async () => {
    setShow(false)
    if (!(await deleteDialog("Are you sure you want to delete this integration?"))) {
      return
    }
  ...
  return (
    <div className="cursor-pointer relative select-none" onClick={changeShow}>
      ...
      {show && (
        ...
        >
          <div className="flex gap-[12px] items-center" onClick={deleteChannel(tParty.id)}>
            <div>
            ...
            <div className="text-[12px]">{t("delete_integration", "Delete Integration")}</div>
          </div>
        </div>
      )}
    </div>
        ...
        >
          <div className="flex gap-[12px] flex-col">
            <div className="flex items-center">
              <h2 className="group-[.sidebar]:hidden flex-1 text-[20px] font-[500]">{t("integrations")}</h2>
              <div
              ...
              <div className="flex-1 flex flex-col gap-[14px]">
                <div className={clsx("gap-[16px] flex flex-col relative justify-center group/profile hover:bg-boxHover rounded-e-[8px]")}>
                  {!isLoading && !data?.length ? (
                    ...
                    data?.map((p: any) => (
                      <div key={p.id} className={clsx("flex gap-[8px] items-center")}>
                        <div className="h-full w-[4px] -ms-[12px] rounded-s-[3px] opacity-0 group-hover/profile:opacity-100 transition-opacity">
                          <SVGLine />
                        </div>
                        <div
                          className={clsx("relative rounded-full flex justify-center items-center bg-fifth")}
                        ...
                        <div
                          // @ts-ignore
                          role="Handle"
                          className={clsx("flex-1 whitespace-nowrap text-ellipsis overflow-hidden group-[.sidebar]:hidden")}
                          data-tooltip-id="tooltip"
                          data-tooltip-content={p.title}
                        >
                          {p.name}
