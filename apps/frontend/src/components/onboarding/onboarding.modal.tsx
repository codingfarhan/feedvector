...
export const OnboardingModal: FC<OnboardingModalProps> = ({ onClose }) => {
  ...
  const completeOnboarding = useCallback(async () => {
    ...
    onClose()
  }, [audience, connectedLinkedIn, fetch, goal, mutate, onClose, role, toaster, websiteUrl])
      ...
      <div className="flex w-full max-w-[860px] bg-newBgColorInner rounded-[20px] flex-col relative max-h-[calc(100dvh-2rem)] sm:max-h-[calc(100dvh-3rem)] overflow-hidden">
        <div className="flex-1 flex overflow-y-auto p-4 sm:p-8 md:p-10">
          <div className="flex flex-col gap-[24px] flex-1 min-w-0">
            ...
            {step === "loading" && <OnboardingLoadingStep onComplete={completeOnboarding} />}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-1 flex-col justify-center gap-[24px] py-[8px]">
      ...
      <div className="flex flex-col gap-[8px]">
        ...
        <input
          ...
          maxLength={2048}
          className="h-[48px] rounded-[8px] border border-newTableBorder bg-newTableHeader px-[14px] text-[14px] text-newTextColor outline-none focus:border-[#8b5cf6]"
          autoFocus
        />
      </div>
      ...
      <div className="flex justify-center pt-[8px]">
        <button
          onClick={onFinish}
          className="flex items-center justify-center gap-[12px] bg-gradient-to-r from-[#622aff] to-[#8b5cf6] text-white font-semibold px-[24px] sm:px-[32px] py-[14px] rounded-[12px] text-[16px] transition-all shadow-lg shadow-purple-500/25 w-full sm:w-auto hover:from-[#7c3aff] hover:to-[#9d6eff] hover:shadow-purple-500/40"
        >
          Finish setup
        </button>
      </div>
