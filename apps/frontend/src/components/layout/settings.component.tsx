...
import { ContentProfileComponent } from "@gitroom/frontend/components/settings/content-profile.component"
export const SettingsPopup: FC<{
  ...
  const list = useMemo(() => {
    ...
    arr.push({ tab: "content_profile", label: t("content_profile", "Content Profile") })
  ...
  const openMobileSettingsMenu = useCallback(() => {
    modal.openModal({
      ...
      children: (
        <div className="h-full w-full bg-newBgColorInner text-newTextColor">
          <div className="h-[56px] px-[14px] flex items-center gap-[10px] border-b border-newTableBorder">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[16px] font-[700] truncate">{t("settings_sections", "Settings sections")}</div>
            </div>
          </div>
          ...
          <div className="px-[14px] py-[14px] overflow-y-auto h-[calc(100dvh-56px)]">
              ...
              {list.map(({ tab: tabKey, label }) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      "w-full text-left flex items-center gap-[12px] px-[14px] py-[12px] rounded-[14px] border border-transparent",
                      isActive ? "bg-boxFocused text-newTextColor border-newTableBorder" : "bg-newTableHeader text-textItemBlur",
                    )}
                  >
                    <div className={clsx("h-[18px] w-[4px] rounded-[3px]", isActive ? "bg-[#612bd3]" : "bg-transparent")} />
                    <div className="text-[16px] font-[600] truncate">{label}</div>
                  </button>
                )
              })}
            </div>
            ...
            {showLogout && (
              <div className="pt-[10px] border-t border-newTableBorder">
                <div className="text-[12px] uppercase tracking-wide text-textItemBlur mb-[10px]">{t("account", "Account")}</div>
                <div className="bg-newTableHeader rounded-[14px] px-[14px] py-[12px]">
                  <LogoutComponent />
                </div>
              </div>
            )}
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-[12px] flex-1 min-w-0">
      <div className="hidden sm:flex bg-newBgColorInner p-[20px] flex-col transition-all w-[260px]">
      ...
      <div className="bg-newBgColorInner flex-1 flex-col flex p-[14px] sm:p-[20px] gap-[12px] min-w-0">
        <div className="sm:hidden">
          <button
            type="button"
            onClick={openMobileSettingsMenu}
          ...
          >
            <div className="min-w-0 text-left">
              <div className="text-[14px] text-textItemBlur">{t("settings", "Settings")}</div>
              <div className="text-[16px] font-[700] truncate">{currentTabLabel}</div>
            </div>
        ...
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(submit)}>
            ...
            <div className={clsx("w-full mx-auto gap-[24px] flex flex-col relative", !getRef && "rounded-[4px]")}>
              {tab === "global_settings" && (
                <div>
                  <GlobalSettings />
                </div>
...
export const SettingsComponent = () => {
  ...
  return (
    <Link href="/settings" onClick={openModal}>
      <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" className="cursor-pointer relative z-[200]">
        <path
