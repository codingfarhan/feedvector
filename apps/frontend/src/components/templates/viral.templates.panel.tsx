...
import {
  ...
  type Platform,
  type Template,
} from '@gitroom/frontend/components/templates/viral.templates.data';
...
const PlatformToggle = ({
  ...
  const itemClass = (active: boolean) =>
    clsx(
      'px-[12px] py-[10px] rounded-[12px] text-[13px] font-[700] transition-colors select-none flex items-center gap-[8px] border',
      active
        ? 'bg-boxFocused text-textItemFocused border-newTableBorder'
        : 'bg-newBgColorInner text-textItemBlur border-newTableBorder hover:text-textItemFocused hover:bg-boxHover'
    );
  ...
  return (
    <div className="flex gap-[8px]">
      <button
        type="button"
        className={itemClass(value === 'linkedin')}
      ...
      <div className="p-[14px] flex items-start gap-[12px]">
        <div className="flex-1">
          <div className="flex items-center gap-[10px]">
            <div className="text-[14px] font-[800] text-newTextColor">
              {tpl.title}
            </div>
      ...
      <div className="px-[14px] pb-[14px] flex gap-[10px]">
        ...
        <button
          ...
          className={clsx(
            ...
            showExample
              ? 'bg-boxFocused text-textItemFocused hover:opacity-90'
              : 'bg-newBgLineColor text-newTextColor hover:bg-boxHover'
          )}
        >
  ...
  return (
    ...
    >
      <div className="p-[14px] blur-sm pointer-events-none select-none">
        ...
        <div className="mt-[10px] whitespace-pre-wrap text-[13px] leading-[1.55] text-newTextColor max-h-[220px] overflow-hidden">
          {tpl.template}
        </div>
      </div>
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="bg-newBgColorInner/90 border border-newTableBorder rounded-[12px] px-[18px] py-[14px] text-center max-w-[360px] shadow-lg">
          <div className="text-[15px] font-[800] text-newTextColor mb-[4px]">
            {`Locked`}
          </div>
...
export const ViralTemplatesPanel = () => {
  ...
  const [xStyle, setXStyle] = useState<string>('All');
  const [xGoal, setXGoal] = useState<string>('All');

  const categories = categoriesByPlatform[platform];
  ...
  const xGoalOptions = useMemo(() => {
    const goals = Array.from(
      new Set(
        templates
          .filter((tpl) => tpl.platform === 'x')
          .map((tpl) => tpl.goal)
          .filter(Boolean)
      )
    ) as string[];
    ...
    // Stable order for UX
    const order = ['engagement', 'growth', 'authority', 'announcement', 'sales'];
    goals.sort((a, b) => order.indexOf(a) - order.indexOf(b));
    return ['All', ...goals];
  }, []);

  const goalLabel = useCallback((g: string) => {
    switch (g) {
      case 'engagement':
        return 'Engagement';
      case 'growth':
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter((tpl) => {
      if (tpl.platform !== platform) return false;

      if (platform === 'x') {
        if (xGoal !== 'All' && tpl.goal !== xGoal) return false;
        if (xStyle !== 'All' && tpl.style !== xStyle) return false;
        return true;
      }
  ...
  }, [platform, category, xGoal, xStyle]);
  ...
  const gated = useMemo(() => {
    ...
    const lockedPreview = filteredTemplates.slice(
      freeUnlockedCount,
      freeUnlockedCount + freeLockedPreviewCount
    );
    const lockedRemaining = Math.max(
      0,
      filteredTemplates.length - freeUnlockedCount - freeLockedPreviewCount
    );
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="p-[20px] border-b border-newTableBorder">
        <div className="text-[18px] font-[800]">
          {t('viral_templates', 'Viral Templates')}
        </div>
        <div className="mt-[4px] text-[13px] text-textItemBlur">
          {t(
            'viral_templates_subtitle',
            'Pick a platform, choose a category, then start from a proven structure.'
          )}
        </div>
      ...
      <div className="flex-1 p-[20px] overflow-auto bg-newBgLineColor scrollbar scrollbar-thumb-fifth scrollbar-track-newBgColor">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-[12px]">
          ...
          {gated.lockedRemaining > 0 && (
            <LockedMoreCard
              count={gated.lockedRemaining}
              onUpgrade={() => router.push('/billing')}
            />
          )}
        </div>
        ...
        {filteredTemplates.length === 0 && (
          <div className="text-[14px] text-textItemBlur">
            {t('no_templates', 'No templates in this category yet.')}
          </div>
        )}
      </div>
    </div>
