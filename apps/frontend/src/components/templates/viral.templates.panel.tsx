...
export const ViralTemplatesPanel = () => {
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter((tpl) => {
      ...
      return tpl.categories.some((item) => item === category);
    });
  }, [category]);
  ...
  const copyTemplate = useCallback(
    async (tpl: Template) => {
      try {
        ...
        toaster.show(
          t('template_copied', 'Template copied to clipboard'),
          'success'
        );
        ...
        toaster.show(
          t('template_copy_failed', 'Could not copy template'),
          'warning'
        );
      }
    },
      ...
      <div className="p-[20px] border-b border-newTableBorder">
        ...
        <div className="mt-[4px] text-[13px] text-textItemBlur">
          {t(
            ...
            'Select a tone and start from a proven LinkedIn post format.'
          )}
        </div>
