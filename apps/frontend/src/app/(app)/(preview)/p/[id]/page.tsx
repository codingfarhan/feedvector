...
const RenderPreviewDate = dynamicLoad(() => import("@gitroom/frontend/components/preview/render.preview.date").then((mod) => mod.RenderPreviewDate), {
  ssr: false,
})

dayjs.extend(utc)
export const metadata: Metadata = {
  title: `${isGeneralServerSide() ? "FeedVector" : "Gitroom"} Preview`,
  description: "",
}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              <div className="min-w-[55px]">
                <Link href="/" className="text-2xl flex items-center justify-center gap-[10px] text-textColor order-1">
          ...
          <div className="text-sm text-gray-400 flex items-center gap-[20px]">
            ...
            <div className="flex-1">
              {t("publication_date", "Publication Date:")} <RenderPreviewDate date={post[0].publishDate} />
            </div>
          </div>
        </div>
      </div>
              ...
              <div key={String(p.id)} className="relative px-4 py-4 bg-third border border-tableBorder">
                <div className="flex space-x-3">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center space-x-2">
                      <h2 className="text-sm font-semibold">{post[0].integration.name}</h2>
                      <span className="text-sm text-gray-500">@{post[0].integration.profile}</span>
                    </div>
                    <div className="flex flex-col gap-[20px]">
                      ...
                      <div className="flex w-full gap-[10px]">
                        {JSON.parse(p?.image || "[]").map((p: any) => (
                          <div key={p.name} className="flex-1 rounded-[10px] max-h-[500px] overflow-hidden">
